...
The base learner for :class:`poker44_model.mlp_bag.BagMLP`: it consumes the
180-dim sanitization-invariant chunk feature rows (features.py FEATURE_NAMES)
and exposes a scikit-style ``predict_proba``.
...
* The module trains and serves on CPU and is small enough that a few thousand
  chunks x 180 features fit in seconds/epoch.
